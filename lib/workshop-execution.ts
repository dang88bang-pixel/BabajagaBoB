...
import {appendEvent} from "@/lib/control-plane";
...
export function executeWorkshopStep(workshopId:string,action:WorkshopAction){
 ...
 const event=appendEvent("workshop.execution.started",`Workshop ${item.name}: ${action} gestartet`,"RUNNING","agent-workshop",{resource:run.id});
 if(event){addProvenanceNode({id:event.id,kind:"EVENT",label:event.type});addProvenanceEdge({from:event.id,to:run.id,relation:"CAUSED_BY"});}
 ...
 try{
  ...
  addProvenanceNode({id:`ART-${run.id}`,kind:"ARTIFACT_DIGEST",label:run.artifactDigest});
  ...
  const done=appendEvent("workshop.execution.completed",`Workshop ${item.name}: ${updated.stage}`,"COMPLETED","agent-workshop",{resource:run.id,causalParentId:event?.id});
  if(done){addProvenanceNode({id:done.id,kind:"EVENT",label:done.type});addProvenanceEdge({from:done.id,to:run.id,relation:"CAUSED_BY"});}
  ...
  const failed=appendEvent("workshop.execution.failed",`Workshop ${item.name}: ${run.message}`,"ERROR","agent-workshop",{resource:run.id,causalParentId:event?.id});
  if(failed){addProvenanceNode({id:failed.id,kind:"EVENT",label:failed.type});addProvenanceEdge({from:failed.id,to:run.id,relation:"CAUSED_BY"});}
