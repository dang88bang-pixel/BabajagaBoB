...
export function createScenario(x:Omit<Scenario,"id"|"state"|"createdAt"|"updatedAt">){
 if(!x||typeof x!=="object")throw new Error("scenario required");
 if(typeof x.name!=="string"||x.name.trim().length===0)throw new Error("scenario name required");
 if(typeof x.kind!=="string"||x.kind.trim().length===0)throw new Error("scenario kind required");
 const now=new Date().toISOString();const s={...x,id:`SCN-${Date.now()}-${scenarios.length}`,state:"DRAFT" as const,createdAt:now,updatedAt:now};scenarios.unshift(s);persist();observe({type:"simulation.scenario.created",message:`Szenario ${s.id} erstellt`,status:"PLANNING",actor:"AG-SCIENTIST",agentId:"AG-SCIENTIST",action:"simulation.create",resource:s.id,argumentsValue:{kind:s.kind}});return clone(s)}
