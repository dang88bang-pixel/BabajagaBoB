...
export function createErrorIncident(input:Omit<ErrorIncident,"id"|"timestamp"|"status">){const x:ErrorIncident={...input,id:newId(),timestamp:new Date().toISOString(),status:"DETECTED"};incidents.set(x.id,x);recordAudit({actor:x.agentId||"SYSTEM",action:"error.detected",resource:x.id,decision:"ALLOW"},x);documentStep({kind:"ERROR",title:x.id+": "+x.symptom,description:x.incident,status:"ERROR",actor:x.agentId||"SYSTEM",taskId:x.taskId});return structuredClone(x)}
...
export function learnFromError(id:string,summary:string,regressionTestId?:string){const x=incidents.get(id);if(!x)throw new Error("Error incident not found");const k=upsertKnowledge({subject:"Never Again: "+x.id,predicate:"prevention",object:summary,layer:"NEGATIVE",state:"ESTABLISHED",sourceIds:x.evidenceIds});x.knowledgeId=k.id;x.regressionTestId=regressionTestId;x.status=regressionTestId?"REGRESSION_LOCKED":"LEARNED";incidents.set(id,x);recordAudit({actor:x.agentId||"SYSTEM",action:"error.learned",resource:id,decision:"ALLOW"},{knowledgeId:k.id,regressionTestId});return structuredClone(x)}
