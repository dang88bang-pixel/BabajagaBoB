...
export function createApproval(x:Omit<ApprovalRequest,"id"|"status"|"createdAt">){const a={...x,id:`APR-${Date.now()}`,status:"PENDING" as const,createdAt:new Date().toISOString()};requests.push(a);observe({type:"approval.requested",message:`Approval ${a.id} angefordert`,status:"APPROVAL_REQUIRED",actor:x.requestedBy,resource:a.id,taskId:a.taskId,action:"approval.request",decision:"REQUIRE_APPROVAL",argumentsValue:a});recordAudit({actor:x.requestedBy,action:"approval.requested",resource:a.id,decision:"REQUIRE_APPROVAL"},a);return clone(a)}
export function resolveApprovalRequest(id:string,status:"GRANTED"|"DENIED",actor:string){const a=requests.find(x=>x.id===id);if(!a)throw new Error("approval not found");if(a.status!=="PENDING")throw new Error("approval already resolved");a.status=status;a.resolvedAt=new Date().toISOString();a.resolvedBy=actor;observe({type:`approval.${status.toLowerCase()}`,message:`Approval ${id}: ${status}`,status:status==="GRANTED"?"COMPLETED":"BLOCKED",actor,resource:id,taskId:a.taskId,action:`approval.${status.toLowerCase()}`,argumentsValue:a});recordAudit({actor,action:`approval.${status.toLowerCase()}`,resource:id,decision:status},a);return clone(a)}
