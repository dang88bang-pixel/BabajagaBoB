...
export function addAuthorityEdge(edge:AuthorityEdge){if(edge.from===edge.to)throw new Error("self delegation is forbidden");edges.push(structuredClone(edge));persist();return structuredClone(edge)}
export function authorityGraph(){return edges.map(x=>structuredClone(x))}
export function issueCapabilityToken(input:Omit<CapabilityToken,"id">){if(input.issuedBy===input.subject)throw new Error("self-grant is forbidden");const token={...input,id:`CAP-${crypto.randomUUID()}`};tokens.set(token.id,structuredClone(token));persist();return structuredClone(token)}
