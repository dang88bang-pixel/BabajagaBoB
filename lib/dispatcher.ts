...
export type DispatchInput={taskId:string;agentId:string;risk:Risk;sandboxType?:string;idempotencyKey?:string};
...
export async function dispatchTask(input:DispatchInput){
 ...
 const gate=executionGate(task,undefined,getControlState().locked,input.agentId);if(!gate.allowed)throw new Error(`execution blocked: ${gate.reasons.join("; ")}`);
