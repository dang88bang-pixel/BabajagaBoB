...
export async function dispatchTask(input:DispatchInput){
  ...
  const sandboxId=`SB-RUN-${run.id}`;
  const sandbox=await sandboxRuntime.create({id:sandboxId,type:input.sandboxType??"development",network:{mode:"DENY",allowlist:[]},risk:input.risk,limits:{cpuMillicores:1000,memoryMb:1024,storageMb:4096,timeoutMs:300000,processes:32}});
