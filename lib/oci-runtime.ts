...
function runDocker(args:string[],timeoutMs:number):Promise<{code:number|null;stdout:string;stderr:string;timedOut:boolean}>{
 return new Promise((resolve,reject)=>{const child=spawn("docker",args,{shell:false,stdio:["ignore","pipe","pipe"]});let stdout="",stderr="",timedOut=false;child.stdout.on("data",c=>stdout+=String(c));child.stderr.on("data",c=>stderr+=String(c));const timer=setTimeout(()=>{timedOut=true;child.kill("SIGKILL")},timeoutMs);child.once("error",e=>{clearTimeout(timer);reject(e)});child.once("close",code=>{clearTimeout(timer);resolve({code,stdout,stderr,timedOut})})})
}

export class OciContainerRuntimeAdapter{
 constructor(){load()}
 async create(spec:OciContainerSpec):Promise<RuntimeHandle>{
  assertSafeImage(spec.image);if(spec.limits.timeoutMs<=0||spec.limits.memoryMb<=0||spec.limits.cpuMillicores<=0||spec.limits.processes<=0)throw new Error("Invalid sandbox resource limits");if(spec.network==="ALLOWLIST")throw new Error("OCI ALLOWLIST networking is fail-closed until an egress proxy is configured");
  const sandboxId=spec.containerName??`oci-${crypto.randomUUID()}`;assertSafeName(sandboxId);
  const args=["create","--name",sandboxId,"--network","none","--cpus",String(spec.limits.cpuMillicores/1000),"--memory",`${spec.limits.memoryMb}m`,"--pids-limit",String(spec.limits.processes),"--read-only","--cap-drop","ALL","--security-opt","no-new-privileges","--tmpfs","/tmp:rw,noexec,nosuid,size=64m",...(spec.workingDirectory?["--workdir",spec.workingDirectory]:[]),spec.image,...spec.command];
 ...
 async reconcile(){for(const [id,item] of handles){const r=await runDocker(["inspect","--format","{{.State.Running}}",id],10_000).catch(()=>({code:1,stdout:"",stderr:"",timedOut:false}));if(r.code!==0){item.handle.state="FAILED";continue}item.handle.state=r.stdout.trim()==="true"?"RUNNING":"PAUSED"}save();return [...handles.values()].map(x=>structuredClone(x.handle))}
 async start(sandboxId:string){const item=handles.get(sandboxId);if(!item)throw new Error("OCI sandbox not found");const r=await runDocker(["start",sandboxId],Math.min(item.handle.limits.timeoutMs,30_000));if(r.timedOut||r.code!==0)throw new Error(`OCI start failed: ${r.stderr||r.stdout||"unknown error"}`);item.handle.state="RUNNING";save();return structuredClone(item.handle)}
 async execute(sandboxId:string,command:string[]):Promise<OciExecutionResult>{const item=handles.get(sandboxId);if(!item)throw new Error("OCI sandbox not found");if(command.length===0)throw new Error("Execution command is empty");const r=await runDocker(["exec",sandboxId,...command],item.handle.limits.timeoutMs);const accepted=r.code===0&&!r.timedOut;return{accepted,exitCode:r.code,stdout:r.stdout,stderr:r.stderr,timedOut:r.timedOut,message:accepted?"Execution completed":"Execution failed"}}
