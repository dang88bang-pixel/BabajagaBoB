...
export function observe(input:{type:string;message:string;status:Status;actor:string;resource?:string;taskId?:string;causalParentId?:string;action:string;decision?:string;argumentsValue?:unknown;from?:string;to?:string;relation?:"DERIVED_FROM"|"TESTED_BY"|"EXECUTED_IN"|"AUTHORIZED_BY"|"CAUSED_BY"|"PRODUCED"}){
 const causalParentId=input.causalParentId??loadEvents()[0]?.id;
 const event:Event={id:`evt-${crypto.randomUUID()}`,type:input.type,message:input.message,status:input.status,time:new Date().toISOString(),actor:input.actor,resource:input.resource,taskId:input.taskId,causalParentId};
 ...
 recordAudit({actor:input.actor,action:input.action,resource:input.resource,decision:input.decision??"ALLOW",causalParentId},input.argumentsValue??null);
 return event;
}