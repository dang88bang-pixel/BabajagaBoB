...
| Routen-Guards je Methode | TESTED | `tests/security/api-route-contract.test.ts` prüft jede exportierte Methode einzeln — **rekursiv auch verschachtelte Routen**. Sechs GET-Routen (`authority`, `cicd`, `devices`, `governance`, `providers`, `worker`) hatten keinen Guard und sind jetzt `*:read`-geschützt; dieselbe Prüfung deckte zwei verschachtelte Routen ohne Aktionsprüfung auf (`app/api/approvals/center`, `app/api/workshop/execute`), die nachgezogen wurden (Freigabe-Center: `approval:read`/`approval:write`; Werkstatt-Ausführung: `workshop:execute` + Capability `workshop:step`, Nutzdaten über `readJson`/`actionField`) |
| Status-Modell (20 Zustände) | TESTED | `lib/status.ts` als einzige Quelle für Anzeigetext, Farbe, Gruppe und Ergebnis (`Record<Status, StatusMeta>` → fehlender Eintrag ist ein Compile-Fehler); Tabellen der Oberfläche rendern Zustandsspalten durch `isKnownStatus`/`StatusBadge`, jeder Tone hat eine CSS-Regel; `tests/unit/status-model.test.ts` (5 Tests) und `tests/regression/ui-contract.test.ts`; Selbstauskunft zur Laufzeit über `GET /api/observatory` (`statusModel`) |
| Agent Observatory | TESTED | `lib/observatory.ts` + `GET /api/observatory`: neun Felder je Aktivität (Ziel, Beobachtung, Hypothese, Aktion, Erwartung, Ergebnis, Evidenz, Schlussfolgerung, nächster Schritt) als reine Projektion über Ereignis-Log und Stores; fehlende Felder werden je Aktivität als `gaps` benannt; UI-Abschnitt „Observatory“ (live: 31 Zeilen, alle 11 Spalten belegt); `tests/integration/observatory-why.test.ts` |
| „Warum?“-Record | TESTED | `GET /api/events/[id]/why`: Zweck, Entscheidung, Akteur, Autorisierungs-/Provenance-Referenz, Kausalkette bis zur Wurzel, Evidenz und Wissen — **keine** Gedankenkette; nicht beantwortbare Teile stehen in `limitations`; 400 für ungültige IDs, 404 für unbekannte Ereignisse; `tests/integration/observatory-why.test.ts`, live über `scripts/audit-api.sh` Abschnitt 8 |
| Live-Nachweis über HTTP | VERIFIED | `scripts/verify-live.sh`: **173 Prüfungen / 0 Fehler** gegen `npx next start` (frisch initialisiert, 2026-09-25; gemessen **174** auf der bereits initialisierten Instanz; ohne cgroup-Delegation **167 / 165**; mit zweitem Faktor **174 / 172**), jeweils mit aktiver Kernel-Isolation und durchgesetzten Ressourcenlimits; alle drei §49-Abnahmen, Agentenweg über Capability-Token ohne Browser-Session, **Evidenz einer blockierten Autorisierung** (`kind=DENIAL`, Digest erneut geprüft, keine Klartext-Argumente), **Replay-Verweigerung** (zweiter Lauf mit demselben Token → 409 + Evidenz), **gemessene Kernel-Isolation** (Schritt 11), **Lockdown blockiert auch interne Läufe** (Schritt 7: `fix.verify` → 409 + Gate-Grund, nach Freigabe bestanden) und **TOTP live** (Schritt 12: Pflicht, Ablehnung, Akzeptanz, Replay) |
...
| Control Center (42 Abschnitte) | TESTED | `tests/ui/control-center.test.tsx` (vollständige Navigation, echte Daten, Anmeldemaske) und `tests/ui/control-center-api.test.tsx` (39 echte Routen-Handler, Metriken als Prometheus-Text, Secrets ohne Lesezugriff); jede Seite ist an eine reale Serverroute gebunden, leer = „keine Einträge“, fehlend = „nicht verfügbar“; live alle Routen mit 200 geprüft |
...
| Automatisierte Testsuiten | TESTED | **59 Dateien / 391 Tests grün** (Unit 104, Security 128, Integration 120, Regression 15, UI 13, E2E 11), siehe `docs/TESTING.md` |
