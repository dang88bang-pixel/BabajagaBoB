...
| Live-Nachweis über HTTP | VERIFIED | `scripts/verify-live.sh`: **171 Prüfungen / 0 Fehler** gegen `npx next start` (frisch initialisiert, 2026-09-25; **169** bei bereits initialisierter Instanz; ohne cgroup-Delegation **168 / 166**; mit zweitem Faktor **177 / 175**), jeweils mit aktiver Kernel-Isolation und durchgesetzten Ressourcenlimits; alle drei §49-Abnahmen, Agentenweg über Capability-Token ohne Browser-Session, **Evidenz einer blockierten Autorisierung** (`kind=DENIAL`, Digest erneut geprüft, keine Klartext-Argumente), **Replay-Verweigerung** (zweiter Lauf mit demselben Token → 409 + Evidenz), **gemessene Kernel-Isolation** (Schritt 11) und **TOTP live** (Schritt 12: Pflicht, Ablehnung, Akzeptanz, Replay) |
...
| Automatisierte Testsuiten | TESTED | **33 Dateien / 186 Tests grün** (Security-Suite 13 / 73), siehe `docs/TESTING.md` |
