...
| Live-Nachweis über HTTP | VERIFIED | `scripts/verify-live.sh`: **174 Prüfungen / 0 Fehler** gegen `npx next start` (frisch initialisiert, 2026-09-25; **169** bei bereits initialisierter Instanz; ohne cgroup-Delegation **168 / 166**; mit zweitem Faktor **177 / 175**), jeweils mit aktiver Kernel-Isolation und durchgesetzten Ressourcenlimits; alle drei §49-Abnahmen, Agentenweg über Capability-Token ohne Browser-Session, **Evidenz einer blockierten Autorisierung** (`kind=DENIAL`, Digest erneut geprüft, keine Klartext-Argumente), **Replay-Verweigerung** (zweiter Lauf mit demselben Token → 409 + Evidenz), **gemessene Kernel-Isolation** (Schritt 11), **Lockdown blockiert auch interne Läufe** (Schritt 7: `fix.verify` → 409 + Gate-Grund, nach Freigabe bestanden) und **TOTP live** (Schritt 12: Pflicht, Ablehnung, Akzeptanz, Replay) |
...
| Automatisierte Testsuiten | TESTED | **42 Dateien / 269 Tests grün** (Unit 53, Security 103, Integration 84, Regression 14, UI 11, E2E 4), siehe `docs/TESTING.md` |
| Betriebsprüfung aller Routen | VERIFIED | `scripts/audit-api.sh`: **201 Prüfungen / 0 Fehler**, Exit 0 (7 Abschnitte, wiederholbar; Routen mit Pflichtparametern werden als 4xx korrekt bewertet); siehe `docs/TESTING.md` §4b |
...
| Oberflächenprüfung des Control Centers | VERIFIED | `scripts/audit-ui.mjs`: **82 Prüfungen / 0 Fehler**, Exit 0 — Quellvertrag, Auslieferung, Visualisierungs-Bildroute (passives SVG) und Datenvertrag je Abschnitt, kein Geheimnis im Browser; siehe `docs/TESTING.md` §4d |
| Vollständige Aktions-/Attributprüfung | VERIFIED | `scripts/audit-actions.mjs`: **488 Prüfungen / 0 Fehler**, Exit 0 — Matrix aus dem Quellcode, Robustheit je Aktion, Attributtypen, 14 Interaktionsketten bis `REGRESSION_LOCKED` inkl. aller Visualisierungsarten; siehe `docs/TESTING.md` §4c |
