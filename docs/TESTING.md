...
| Stufe | Inhalt |
|---|---|
| A. Statik | alle 39 Navigationsabschnitte haben einen Renderpfad (kein leerer Abschnitt), kein Renderzweig ohne Navigationseintrag, jede der 28 Datenquellen zeigt auf eine existierende Route, alle im Client genutzten API-Pfade existieren, **kein literales Markdown im sichtbaren Text**, keine Platzhalterwerte, **jede Tabellenspalte ist ein in `lib/`/`app/` deklariertes Feld**, keine Zugangsgeheimnisse im Clientcode |
