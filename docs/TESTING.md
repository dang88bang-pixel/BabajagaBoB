...
**Letzter verifizierter Lauf:** `npx vitest run` → **59 Dateien, 391 Tests, alle grün**
(Unit 104, Security 128, Integration 120, Regression 15, UI 13, E2E 11); `./node_modules/.bin/tsc --noEmit` fehlerfrei; `npx eslint .` 0 Fehler / 10 Warnungen; `npm run build` erfolgreich.
...
(abgeschwächte Schutzregeln) → **19/19 Proben erkannt** (mit vorgeschalteter Grundprobe: alle 18
Suiten der Proben laufen zuerst **ohne** Mutation grün).
...
Ergebnis: **19/19 Proben erkannt**. Regeln des Prüfers:
