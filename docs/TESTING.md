...
**Letzter verifizierter Lauf:** `npx vitest run` → **51 Dateien, 314 Tests, alle grün**
(Unit 74, Security 109, Integration 96, Regression 15, UI 12, E2E 8); `./node_modules/.bin/tsc --noEmit` fehlerfrei; `npx eslint .` 0 Fehler / 10 Warnungen; `npm run build` erfolgreich.
...
| `tests/ui` | 2 | 12 | Control-Center-Oberfläche unter jsdom: **vollständige Navigationsliste (40 Abschnitte)** vorhanden, echte Serverdaten werden als Zeilen gerendert (kein Platzhalter „READY“), fehlende Daten werden ausdrücklich als „nicht verfügbar“ gemeldet, Geräte weisen „nein — Discovery ≠ Autorisierung“ aus, Betrieb/Persistenz zeigt die Backup-Automation mit Intervall, Aufbewahrung und Idempotenzhinweis, Metriken zeigt die geprüften Alarmregeln, ohne Session erscheint die Anmeldemaske und keine Rohdaten; zweite Datei ruft die **echten Routen-Handler** auf (38 Routen) und prüft, dass gerenderte Datensätze tatsächlich aus der API stammen (inkl. Alarmregeln der echten Route und Backup-Automation) |
| `tests/e2e` | 3 | 8 | Kette Creator → Aufgabe → Autorisierung → Sandbox → Ausführung → Evidence → Knowledge sowie Fehlerkette DETECTED → DIAGNOSING → EXPERIMENTING → ROOT_CAUSE_FOUND → FIXING → VERIFYING → LEARNED → REGRESSION_LOCKED; **Abnahmekette** (`acceptance-chain.test.ts`: alle 18 Stufen über API, Persistenz, Audit/Event und Provenance) |
...
**Letzter Lauf (2026-09-25):** `174 bestanden, 0 fehlgeschlagen`, Exit 0 — auf der
Frischinstanz `:3100` mit cgroup-Delegation und Kernel-Isolation (vorher mehrfach
...
| Abschnitt | Inhalt |
|---|---|
| Statik | 38 POST-Routen, **116 Aktionen** und alle im Body gelesenen Attribute werden aus dem Code gelesen |
...
`scripts/audit-ui.mjs` **89 bestanden, 0 fehlgeschlagen** (45 Datenabrufe, 29 Abschnitte mit
