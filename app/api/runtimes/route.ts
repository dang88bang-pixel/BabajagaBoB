...
export async function POST(req:Request){
 requireControlPlaneAuth(req);try{return NextResponse.json({runtime:registerRuntime(await req.json())},{status:201})}catch(e){return NextResponse.json({error:e instanceof Error?e.message:"invalid runtime"},{status:400})}}
