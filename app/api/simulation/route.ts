...
export async function POST(request:Request){
 ...
 try{const b=await request.json();if(b.action==="create")return NextResponse.json(createScenario(objectField(b,"scenario") as unknown as Parameters<typeof createScenario>[0]),{status:201});if(b.action==="advance")return NextResponse.json(advanceScenario(String(b.id),b.state,b.result));return NextResponse.json({error:"Unsupported simulation action"},{status:400})}catch(e){return NextResponse.json({error:e instanceof Error?e.message:"simulation error"},{status:400})}}