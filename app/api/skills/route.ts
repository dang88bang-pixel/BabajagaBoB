...
export async function POST(req:Request){
 ...
 try{const b=await req.json();if(b.action==="register")return NextResponse.json({skill:registerSkill(objectField(b,"skill") as unknown as Parameters<typeof registerSkill>[0])},{status:201});if(b.action==="transition")return NextResponse.json({skill:transitionSkill(b.id,b.lifecycle)});return NextResponse.json({error:"unknown action"},{status:400})}catch(e){return NextResponse.json({error:e instanceof Error?e.message:"invalid skill"},{status:400})}}
