...
export async function POST(req:Request){try{const b=await readJson(req);const action=actionField(b,["delegate","issue","revoke"]);const token=typeof b.capabilityTokenId==="string"?b.capabilityTokenId:undefined;requireCapability(token,action==="delegate"?"authority:delegate":action==="issue"?"authority:issue":"authority:revoke");
